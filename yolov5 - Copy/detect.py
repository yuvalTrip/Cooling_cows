...
def extract_name_precentage(input_string):
        # Split the string by spaces
        parts = input_string.split()
        # Extract the first element (name) and the second value (precentage)
        name = parts[0]
        value = float(parts[1])
...
def run(
    ...
    for image_idx, coordinates_and_labels_for_image in enumerate(all_coordinates):
        ...
        for idx, xyxy in enumerate(coordinates_and_labels_for_image):
            ...
            name, value =extract_name_precentage(xyxy)
            if name == 'cow' and value> 0.5 :
                cow_num= cow_num+1
        # To change the lower bound (i.e the minimum number of cows to start colling the barn)of cow_num, change o here:
        if cow_num > 0:
            print (f"There are {cow_num} cows. Start cooling.")
        else:
            print (f"There are not enough cows to start cooling.")
